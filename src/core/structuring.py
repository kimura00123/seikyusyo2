...
3. 明細の抽出ルール：
   ...
   - 各明細に関連する追加情報（日付範囲、在庫に関する情報、数量に関する情報）も漏れなく抽出
     - 在庫に関する情報: 摘要や補足情報から「繰越」「入庫」「出庫」「残」などの在庫関連の数値があれば、それらをまとめて記録してください。該当情報がなければ、在庫に関する情報は記録不要です。
     - 数量に関する情報:
       - 各明細について、原則として数量と、もしあればその単価を記録してください。
       - 数量は、明細の摘要や補足情報から「数量 XX」（XXは数値）、「XX箱」、「XX個」などの記載があればその数値を抽出します。
       - 数量の概念が適用できない、または読み取れない場合に限り、数量に関する情報は記録不要です。
       - 在庫に関する情報と数量に関する情報は異なる概念です。混同しないように注意してください。
...
4. データ形式：
   ...
   - **数値項目全般: 数値のみを抽出（カンマは除去）。抽出できない場合は `null` とするが、後処理で0に変換されることを想定。**
...
特に重要:
- 金額や数量などの数値は必ず数値のみを抽出する。¥記号やカンマ(,)など、数字以外の文字は全て取り除くこと。
